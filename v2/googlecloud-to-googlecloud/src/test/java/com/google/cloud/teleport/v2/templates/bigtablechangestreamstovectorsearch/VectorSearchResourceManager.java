...
// TODO(meagar): :%s/LOG\.info/LOG.debug
...
public class VectorSearchResourceManager implements ResourceManager {
  ...
  public static final String TEST_INDEX_NAME = "foo5-cbt-cdc-index"; // -no-kill";
  public static final String TEST_ENDPOINT_NAME = "foo5-cbt-cdc-endpoint"; // -no-kill";
  public static final String TEST_DEPLOYED_INDEX_NAME = "foo5_cbt_cdc_deploy"; // _nokill_no_kill";

  //  public static final String TEST_INDEX_NAME = "meagar-test-index"; // -no-kill";
  //  public static final String TEST_ENDPOINT_NAME = "meagar-test-index-endpoint"; // -no-kill";
  //  public static final String TEST_DEPLOYED_INDEX_NAME = "meagar_test_deploy2_1709829816174"; //
  // _nokill_no_kill";
  ...
  // TODO(meagar): Do we need to actually expose this?
  // public DeployedIndex getTestDeployedIndex() { return testDeployedIndex; }
  ...
  public IndexDatapoint findDatapoint(String datapointID) {
    LOG.info("Finding datapoint {}", datapointID);
    ...
    try {
      ...
      LOG.info("Datapoint {} does not exist (NotFoundException)", datapointID);
      return null;
    }

    for (var i : response.getDatapointsList()) {
      if (i.getDatapointId().equals(datapointID)) {
        LOG.info("Datapoint {} Exists", datapointID);
        return i;
      }
    }
  ...
  public void addDatapoint(String datapointId, Iterable<Float> vector) {
    ...
    LOG.info("Done thing");
    // LOG.info("Update Mask", dpr.hasUpdateMask());
  ...
  private Index findOrCreateIndex() throws Exception {
    LOG.info("Doing find-or-create for test index {}", TEST_INDEX_NAME);
    for (var i : indexClient.listIndexes(parent).iterateAll()) {
      if (i.getDisplayName().equals(TEST_INDEX_NAME)) {
        LOG.info("Using existing index: {}:{}", i.getDisplayName(), i.getName());
        return i;
      } else {
        LOG.info("Ignoring index: {}:{}", i.getDisplayName(), i.getName());
      }
    }

    LOG.info("Index {} does not exist, creating", TEST_INDEX_NAME);
    return createIndex(TEST_INDEX_NAME);
  }

  private IndexEndpoint findOrCreateEndpoint() throws Exception {
    LOG.info("Doing find-or-create for test endpoint {}", TEST_ENDPOINT_NAME);

    for (var e : endpointClient.listIndexEndpoints(parent).iterateAll()) {
      if (e.getDisplayName().equals(TEST_ENDPOINT_NAME)) {
        LOG.info("Using existing endpoint: {}:{}", e.getDisplayName(), e.getName());
        return e;
      } else {
        LOG.info("Ignoring endpoint {}:{}", e.getDisplayName(), e.getName());
      }
    }

    LOG.info("Endpoint {} does not exist, creating", TEST_ENDPOINT_NAME);
    return createEndpoint(TEST_ENDPOINT_NAME);
  }

  private DeployedIndex findOrCreateDeployedIndex() throws Exception {
    LOG.info("Doing find-or-create for test index deployment {}", TEST_DEPLOYED_INDEX_NAME);

    for (var d : testEndpoint.getDeployedIndexesList()) {
      if (d.getId().equals(TEST_DEPLOYED_INDEX_NAME)) {
        LOG.info("Using existing deployment: {}:{}", d.getDisplayName(), d.getId());
        return d;
      } else {
        LOG.info("Ignoring deployment {}:{}", d.getDisplayName(), d.getId());
      }
    }

    LOG.info("DeployedIndex {} does not exist, creating", TEST_DEPLOYED_INDEX_NAME);
  ...
  private Index createIndex(String indexName) throws Exception {
    ...
    var index =
        Index.newBuilder()
            ...
            // TODO(meagar): Hardcode the following, or make it configurable?
        ...
        DeployIndexRequest.newBuilder()
            ...
            .setDeployedIndex(
                DeployedIndex.newBuilder()
                    ...
                    .setId(TEST_DEPLOYED_INDEX_NAME) // TODO(meagar): What is this? Do we have to
